...
impl ExecutorAdapter for OpenCodeAdapter {
    ...
    fn start_run(
        ...
        generic_cli::start_run("opencode", self.binary_name(), request, &["run", "{prompt}"], on_event)
    }

    fn spawn_session(&self, request: &ExecutorRunRequest) -> Result<ExecutorSession> {
        generic_cli::spawn_session(self.binary_name(), request, &["run", "{prompt}"])
    }
...
mod tests {
    ...
    fn executes_with_configured_template_override() {
        ...
        let session = adapter
            .start_run(
                &ExecutorRunRequest {
                    ...
                    config_json: json!({
                        "run_template": ["-lc", "printf opencode"]
                    }),
        ...
        assert_eq!(events[0].event_type, "executor.opencode.started");
        assert_eq!(session.raw["stdout"], "opencode");
    }
}
